...
resource "null_resource" "get_cluster_credentials" {
  provisioner "local-exec" {
    command = "gcloud container clusters get-credentials ${data.google_container_cluster.cluster.name} --region ${data.google_container_cluster.cluster.location} --project ${var.google_cloud_project}"
  }

  depends_on = [google_container_cluster.cluster]
}

resource "null_resource" "add_wIU_iam_policy" {
  provisioner "local-exec" {
    command = <<-EOT
    gcloud iam service-accounts add-iam-policy-binding \
     --role="roles/iam.workloadIdentityUser" \
     --member="serviceAccount:${var.google_cloud_project}.svc.id.goog[default/gke-ksa]" \
     ${google_service_account.test_service_account.email}
     EOT

  }
  depends_on = [kubernetes_service_account.gke_ksa_service_account]
}
